...
@author: jacob
'''
from PyQt5.Qt import QDialog, QGridLayout
from PyQt5.QtGui import QTextCursor, QTextDocument
from PyQt5.QtWidgets import QWidget, QPushButton, QRadioButton, QTextEdit, QLabel, QCheckBox
...
class Find(QDialog):
    ...
    def find(self):
        ...
        #my own way of finding words with using the built in search method
        '''
        if self.wholeWords.isChecked():
            #checking to make sure that method worksr 
            answer = self.parent.textEdit.find(query, QTextDocument.FindWholeWords) 
            if answer:
                print("Heck Yeah")
        elif self.caseSens.isChecked():
            answer = self.parent.textEdit.find(query, QTextDocument.FindCaseSensitively)
            if answer:
                print("Heck Yeah")            
        else:
            self.parent.textEdit.find(query, QTextDocument.FindBackward)  
        ...
        if self.normalRadio.isChecked(): 
            document = self.parent.textEdit.document()
            
            textcursor = self.parent.textEdit.textCursor()
            
            cursor = document.find(query, textcursor, QTextDocument.FindBackward)
